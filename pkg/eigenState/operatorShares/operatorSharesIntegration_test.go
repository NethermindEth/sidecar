...
func hydrateOperatorShareTransactionLogs(grm *gorm.DB, l *zap.Logger) error {
	projectRoot := getProjectRootPath()
	contents, err := tests.GetOperatorSharesTransactionLogsSqlFile(projectRoot)
...
func Test_OperatorSharesIntegration(t *testing.T) {
	...
	t.Run("Operator shares calculation", func(t *testing.T) {
		...
		err = hydrateOperatorShareTransactionLogs(grm, l)
		assert.Nil(t, err)
		...
		var count int
		query = `select count(*) from operator_share_deltas`
