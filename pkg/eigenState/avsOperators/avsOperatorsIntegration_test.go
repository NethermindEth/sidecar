...
func hydrateTransactionLogs(grm *gorm.DB, l *zap.Logger) error {
	projectRoot := getProjectRootPath()
	contents, err := tests.GetAvsOperatorsTransactionLogsSqlFile(projectRoot)
...
func Test_AvsOperatorsIntegration(t *testing.T) {
	...
	t.Run("AvsOperator registrations", func(t *testing.T) {
		...
		fmt.Printf("Total avs_operator_state_changes: %d\n", count)
		assert.Equal(t, 2121, count)
	})
